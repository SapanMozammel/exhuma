...
export async function addCommand(
  ...
  const flavor: EcosystemFlavor = (options.flavor && SUPPORTED_ECOSYSTEMS.includes(options.flavor as EcosystemFlavor))
    ? (options.flavor as EcosystemFlavor)
    : config?.flavor || detectEcosystem();
  ...
  for (const slug of selectedSlugs) {
    ...
    try {
      ...
      spinner.succeed(
        `${pc.green('Installed')} ${pc.bold(compName)} → ${pc.dim(targetDir)}`
      );
    } catch (err: any) {
