	...
	const handlePointerDown = useCallback(
		(e: React.PointerEvent<HTMLDivElement>) => {
			...
			const newPos = isVertical
				? calculateVerticalSplitPosition(e.clientY, rect.top, rect.height)
				: calculateSplitPosition(e.clientX, rect.left, rect.width);
	...
	return (
		<div
			...
			className={`border-border focus-visible:ring-primary relative overflow-hidden rounded-2xl border select-none focus:outline-none focus-visible:ring-2 ${
				disabled ? 'opacity-60 cursor-not-allowed' : isVertical ? 'cursor-ns-resize' : 'cursor-ew-resize'
			} ${className}`}
			style={{ aspectRatio }}
		>
			...
			<div
				ref={beforeLayerRef}
				className='pointer-events-none absolute inset-0 size-full will-change-[clip-path]'
			...
			>
				...
				<div
					...
					} bg-white shadow-[0_0_10px_rgba(0,0,0,0.5)]`}
				/>

				{/* Center Handle Button */}
				<div
					className={`border-border bg-background text-muted-foreground absolute flex h-8 w-8 items-center justify-center rounded-full border shadow-lg ${handleClassName}`}
				>
					{isVertical ? (
